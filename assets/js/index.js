...
var inputProductDescription = document.getElementById(
  "inputProductDescription1",
);
...
window.onload = function () {
  if (
    localStorage.getItem("specialProducts") != null &&
    JSON.parse(localStorage.getItem("specialProducts")).length
  ) {
...
function clearForm() {
  for (let i = 0; i < productInputs.length; i++) {
    ...
    if (productInputs[i].parentElement.childElementCount == 3) {
      //This next lines is used to remove the paragraph with the error message
      productInputs[i].parentElement.removeChild(
        productInputs[i].parentElement.children[2],
      );
    }
  }
...
function displayProducts() {
  ...
  for (let i = 0; i < products.length; i++) {
    holder += renderCard(products[i], i);
  }
...
function renderCard(product, index) {
  return `
    <div class="special-product-item m-3">
                <div class="special-product-image position-relative">
                    <img src="${product.imageUrl}" alt="${
    product.productName
  }" class="special-product-img w-100">
                    <div class="position-absolute special-products-icons w-100 text-center">
                        <i class="fa-solid fa-eye mx-1 p-2 shadow-lg bg-white rounded" onclick="popupScreen(${index})"></i>
                    </div>
                </div>
                <p class="special-product-name text-center my-3">${
                  product.productName
                }</p>
                <div class="special-product-info text-center pb-3 position-relative">
                    <div class="stars text-warning my-4">
                        ${findStars(product.numberOfStars)}
                    </div>
                    <div class="currentPrice me-2">Discount: ${
                      product.discount
                    }%</div>
                    <span class="currentPrice">Real Price: $${printPrice(
                      `${product.realPrice}`,
                    )}</span>
                    <div class="special-product-buttons d-flex flex-column flex-md-row product-${index} w-100 h-100 position-absolute justify-content-evenly align-items-center">
                        <button type="button" onclick="deleteProduct(${index})" class="btn btn-danger w-35">Delete</button>
                        <button type="button" onclick="fillForm(${index})" class="btn btn-secondary w-35">Update</button>  
                    </div>
                </div>
    </div>
    `;
